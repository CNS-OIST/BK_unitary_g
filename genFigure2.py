...
Supp = True # turn on to generate Supplementary Figure 3

if Supp: mesh = 'Cylinder3dia5umL10um'
else: mesh = 'Cylinder3dia1umL10um'
...
for CaP_fac in CaP_facs:
    ...
    with HDF5Handler('./STEPS/data_CaPstates/'+dataset) as hdf:
        ...
        CaP_maxes.append(np.max(CaPstates.data[0,:,1]*CaP_g))
...
for i in range(len(CaP_facs)): axins1.plot(CaP_facs[i], CaP_maxes[i]/CaP_maxes[0], 'o')
...
if Supp: plt.savefig(f'Figures_Sup/FigureS3.pdf', dpi=300)
else: plt.savefig(f'Figures/Figure2.pdf', dpi=300)
#plt.show()
plt.close()
