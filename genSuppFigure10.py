...
for CaP_fac in CaP_facs:
    ...
    with HDF5Handler('./STEPS/data_CaPstates/'+dataset) as hdf:
        ...
        """
        ax1.plot(1e3 * CaPstates.time[0], Currents.data[0,:,3] * 1e9, label = CaP_fac )
        CaP_maxes.append(np.max(-1e9 * Currents.data[0,:,3]))
        ax4.plot(1e3 * CaPstates.time[0], 100*CaPstates.data[0,:,1]/np.sum(CaPstates.data[0,:,:], axis=1), label = CaP_fac )
        #ax4.plot(1e3 * Pot.time[0], 1e3 * Pot.data[0], label = CaP_fac )
...
for BK_fac in BK_facs:
    ...
    with HDF5Handler('./STEPS/data/'+dataset) as hdf:
        ...
        """
        ax2.plot(1e3 * Currents.time[0], 1e9 * Currents.data[0,:,5])
        BK250_maxes.append(np.max(1e9 * Currents.data[0,:,5]))
        ax5.plot(1e3 * BKstates.time[0], 100*actTot/np.sum(BKstates.data[0,:,:], axis=1), label = BK_fac )
        #ax5.plot(1e3 * Pot.time[0], 1e3 * Pot.data[0], label = BK_fac )
...
for BK_fac in BK_facs:
    ...
    with HDF5Handler('./STEPS/data/'+dataset) as hdf:
        ...
        """
        ax3.plot(1e3 * Currents.time[0], 1e9 * Currents.data[0,:,5])
        BK50_maxes.append(np.max(1e9 * Currents.data[0,:,5]))
        ax6.plot(1e3 * BKstates.time[0], 100*actTot/np.sum(BKstates.data[0,:,:], axis=1), label = BK_fac )
        #ax6.plot(1e3 * Pot.time[0], 1e3 * Pot.data[0], label = BK_fac )

axins1 = inset_axes(ax1, width=0.5, height=0.5, bbox_to_anchor=(0.65, .3, .3, .5), bbox_transform=ax1.transAxes)
axins1.plot(range(1,6), range(1,6), 'k--')
for i in range(4): axins1.plot(CaP_facs[i], CaP_maxes[i]/CaP_maxes[0], 'o')

axins2 = inset_axes(ax2, width=0.5, height=0.5, bbox_to_anchor=(0.65, .1, .3, .5), bbox_transform=ax2.transAxes)
axins2.plot(range(1,6), range(1,6), 'k--')
for i in range(4): axins2.plot(BK_facs[i]/BK_facs[0], BK250_maxes[i]/BK250_maxes[0], 'o')

axins3 = inset_axes(ax3, width=0.5, height=0.5, bbox_to_anchor=(0.25, .3, .3, .5), bbox_transform=ax3.transAxes)
...
ax1.legend(CaP_facs, loc='lower right',facecolor='white', framealpha=1)
...
for ax in (ax2, ax3):
    ax.legend(BK_facs, loc='upper right',facecolor='white', framealpha=1)
...
ax5.set_ylabel('% of activated BK250 channels')
ax6.set_ylabel('% of activated BK50 channels')

for axins in (axins1, axins2, axins3):
    #axins.set_ylabel('Peak current')
...
plt.savefig(f'Figures_Sup/FigureS10.pdf', dpi=300)
plt.close()
